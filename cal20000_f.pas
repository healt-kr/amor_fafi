...
begin
  ...
  begin
    ...
    st := st + #13 + '이번달 절입 : '+monthst[inginame]+' 양력' + inttostr(ingiyear)+'년 ';
  ...
  begin
    st := '진짜만세력 V0.93- 안내' + #13+#13;
    st := st + '유효기간 : -10,000년~10,000년(이만년)' + #13;
    st := st + '그레고리오력,태음태양력,일진,24절기,합삭시간,28수,일식,월식 표시'+#13+#13;

    st := st + '팁 : 날짜칸의 빈곳을 누르면 그날의 자세한 사항이 나옵니다.'+#13+#13;
    st := st + '만세력 홈페이지 : http://user.chollian.net/~kohyc/calendar'+#13;
    st := st + '              Email : kohyc@chollian.net';
    //msgcode:=messagedlg(st ,mtinformation,[mbok],0)
    messagedlg(st ,mtinformation,[mbok],0)
  end
end;
